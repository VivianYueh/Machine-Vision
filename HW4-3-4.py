...
# Define the HybridNetWithAttention Model
class HybridNetWithAttention(nn.Module):
    def __init__(self, num_classes, densenet_dropout=0.2, resnet_dropout=0.2):
        super(HybridNetWithAttention, self).__init__()
        
        # Custom DenseNet
        self.densenet = Densenet(DenseBlock, [6, 12, 24, 16], growth_rate=32, num_class=num_classes)
        ...
        self.attention_densenet = SelfAttention(in_dim=1024)  # Adjust the dimension based on your architecture
        self.attention_resnet = SelfAttention(in_dim=num_ftrs_resnet)
        
        # Fully connected layers
        self.densenet.fc = nn.Sequential(
            nn.Dropout(p=densenet_dropout),
            nn.Linear(self.densenet.fc_input_features, num_classes)
        )
        self.resnet.fc = nn.Sequential(
            nn.Dropout(p=resnet_dropout),
            nn.Linear(num_ftrs_resnet, num_classes)
        )
        
        # 1x1 Convolution to adjust channels before merging
        self.adjust_channels_densenet = nn.Conv2d(1, 1024, kernel_size=1, stride=1, padding=0)
        self.adjust_channels_resnet = nn.Conv2d(1, 2048, kernel_size=1, stride=1, padding=0)
        
        # Merge layer
        self.merge_fc = nn.Linear(1024 + 2048, num_classes)
    
    def forward(self, x):
        # Forward pass through DenseNet
        x1 = self.densenet(x)
        x1 = x1.view(x1.size(0), 1, -1, 1)  # Adjust dimensions for attention layer
        x1 = self.attention_densenet(x1).view(x1.size(0), -1, 1, 1)  # Squeeze back to original dimensions
        x1 = self.adjust_channels_densenet(x1)
        
        # Forward pass through ResNet
        x2 = self.resnet(x)
        x2 = x2.view(x2.size(0), 1, -1, 1)  # Adjust dimensions for attention layer
        x2 = self.attention_resnet(x2).view(x2.size(0), -1, 1, 1)  # Squeeze back to original dimensions
        x2 = self.adjust_channels_resnet(x2)
        
        # Concatenate outputs from DenseNet and ResNet
...
def build_hybridnet_with_attention(num_classes, densenet_dropout=0.2, resnet_dropout=0.2):
    model = HybridNetWithAttention(num_classes, densenet_dropout, resnet_dropout)
    return model
...
for i, (train_idx, test_idx) in enumerate(skf.split(img_list, img_labels)):
    ...
    model = build_hybridnet_with_attention(len(label_names),True)
